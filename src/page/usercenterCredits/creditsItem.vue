<template>
    <div class="stroll-item">
        <!-- <div> -->
        <div  class="stroll-type-item" @click="SkipAppItem(item)">
            ...
            <div class="stroll-price-bar">
                ...
                <span class="price" v-else>￥<span class="item-ActivityPrice">{{item.SalePrice}}</span></span>
     ...
     methods:{
        SkipAppItem(item) {
            ...
            //   alert('isAndroid'+item.Code);
                window.OCmodel.gotoItem(item.Code);//调用原生方法
            }
            if (isiOS) {
            //   alert('isiOS'+item.Code);
                OCmodel.gotoItem(item.Code);//调用原生方法
            }
