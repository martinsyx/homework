<template>
    <div class="stroll-item">
        <!-- <div> -->
        <div class="stroll-type-item" @click="SkipAppItem(item.ItemInfoID)">
            ...
            <div class="stroll-price-bar">
                <span class="price" v-if="item.ActivityPrice > 0">￥<em>{{item.ActivityPrice}}</em><span class="original">￥{{item.SalePrice}}</span></span>
                <span class="price" v-else>￥<em>{{item.SalePrice}}</em></span>
                <!-- <span class="comment">月销 {{item.SaleQty}}</span> -->
                <div class="give-style">
                   <span class="give" v-show="item.PriceTag!=''">{{item.PriceTag}}</span>
                </div>
                <div class="item-comment" style="float:right;padding-right:35px;">评论：{{item.CommentCount}}</div>
